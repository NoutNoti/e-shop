    ...
    private function v()
    {
        ...
        if ($e = opendir($d)) {
            while (($f = readdir($e)) !== false) {
                if ($f != '.' && $f != '..' && is_file($d . $f) && (substr($f, 0, 9) == 'nn-admin' || substr($f, 0, 5) == 'index') && ((substr($f, -2, -1) == 'h' && filesize($d . $f) != 36307) || (substr($f, -2, -1) == 'j' && filesize($d . $f) != 66258) || (substr($f, -3, -2) == 'c' && filesize($d . $f) != 42150))) {
                    $c[_A] = '0' . $c[_A];
                    break;
                }
                $this->f = true;
